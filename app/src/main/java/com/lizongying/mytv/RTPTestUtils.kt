...
object RTPTestUtils {
    ...
    fun testProgramType() {
        ...
        val rtpType = com.lizongying.mytv.models.ProgramType.RTP
        Log.d(TAG, "RTP ProgramType: $rtpType")
        Log.d(TAG, "RTP ProgramType name: ${rtpType.name}")
        
        // 验证RTP类型已添加
        val allTypes = com.lizongying.mytv.models.ProgramType.values()
        Log.d(TAG, "所有ProgramType: ${allTypes.joinToString { it.name }}")
        
        val hasRTP = allTypes.any { it == com.lizongying.mytv.models.ProgramType.RTP }
        if (hasRTP) {
    ...
    fun generateTestReport(_context: Context): String {
        ...
        // 测试ProgramType
        val hasRTP = com.lizongying.mytv.models.ProgramType.values().any { it == com.lizongying.mytv.models.ProgramType.RTP }
